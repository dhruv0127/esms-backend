...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const kreddo_app_email = settings['kreddo_app_email'];
  const kreddo_base_url = settings['kreddo_base_url'];

  const url = checkAndCorrectURL(kreddo_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | Kreddo',
    kreddo_app_email,
